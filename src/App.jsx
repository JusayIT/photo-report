...
export default function App() {
  ...
  const generateExcelBlob = async () => {
    ...
    for (const [index, p] of previewDoc.positions.entries()) {
      ...
      let maxImgHeightPx = 0;
      const imagesToInsert = [];
      const maxWidthPx = 180;
      const maxHeightPx = 240; // Максимально допустимая высота картинки
      ...
      if (p.photoInv) {
        ...
        if (dims.width > 0 && dims.height > 0) {
          ...
          if (h > maxImgHeightPx) maxImgHeightPx = h;
        }
      }
      if (p.photoObj) {
      ...
      // Переводим реальную высоту фото из пикселей (px) в пункты (pt) для Excel (1 pt = 0.75 px)
      // Добавляем небольшой отступ в 10 pt для визуальной аккуратности
      const finalRowHeightPt = maxImgHeightPx > 0 ? (maxImgHeightPx * 0.75) + 10 : 40;
      row.height = finalRowHeightPt;
      ...
      imagesToInsert.forEach((imgData) => {
        try {
          ...
          // Вычисляем вертикальный отступ в пикселях для центрирования картинки в ячейке
          const maxRowHeightPx = finalRowHeightPt / 0.75;
          const rowOffset = Math.max(4, Math.round((maxRowHeightPx - imgData.h) / 2));
          
          worksheet.addImage(imageId, {
            tl: { col: imgData.col, row: row.number - 1, colOff: 6, rowOff: rowOffset },
